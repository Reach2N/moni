...
| **Voice capture** | A `useAudioRecorder` hook (`react-audio-voice-recorder` or equivalent) | Hand-rolling MediaRecorder state (start, stop, pause, blob, elapsed, permission denial) is a day of edge cases for a solved problem. Check AI Elements' voice components first so it stays in one system. Must emit `webm` or `wav`: CLAUDE.md records that `audio/mp4` is silently ignored. |
