...
| **Owner calendar** | **schedule-x MIT core**: `@schedule-x/react` 4.1.0, `@schedule-x/calendar` 4.7.0, `@schedule-x/theme-default` 4.7.0, `@schedule-x/events-service` 4.7.0 | Adopted 3 September 2026, replacing the hand-built lane grid this table used to mandate. Day, week and month views, keyboard focus, overlap resolution and a responsive grid, none of it maintained here. Its theme is fed from the Invitation's own tokens in `globals.css`, so nothing in `@schedule-x/theme-default` is edited. It has no resource view: see the Cal.com row below for what that costs and what it does not. |
...
| Candidate | Verdict |
|---|---|
| **Cal.com / cal.diy** | Wrong model, not just heavy. Cal.com does *personal* scheduling: one person, calendar sync, event types. Moni does *resource occupancy*: three chairs, twelve hotel rooms, two repair bays, with a database-level exclusion constraint preventing double-booking. The existing slot calculation already models the right domain. Adopting Cal.com means running a second schema and syncing it. **Still true on 3 September 2026, and the resource MODEL is unchanged**: `resources` is still its own table, `bookings.resource_id` still travels on every row, the exclusion constraint still refuses a double-booked chair, and `getCalendarRange()` still returns both. What was given up that day is only the resource VIEW, one column per chair, because the schedule-x plugin that draws it cannot be obtained through npm at any price short of a licence. The data still knows which chair; the screen shows it as a colour. |
| **FullCalendar** | Not a complete fit for the required resource-lane interaction in the current showcase scope, and its resource timeline is a paid plugin for the same reason schedule-x's is. Settled by the schedule-x adoption above. |
| **schedule-x resource view** (`@schedule-x/resource-scheduler`, `@sx-premium/resource-scheduler`) | The MIT core is ADOPTED, see the table above. Only the resource view is rejected, and not on merit: both package names return 404 from npm, verified again on 3 September 2026, so it is a paid plugin on a private registry rather than something an install can reach. Do not check a fourth time and do not hand-build a replacement. Revisit only if a licence is bought, at which point the same rows drive it: `calendarId` becomes `resourceId` and nothing in `src/lib/queries/calendar.ts` moves. |
